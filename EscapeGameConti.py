...
class simpleapp_tk(Tk):
    ...
    def pre_initialize(self):
        ...
        self.lbl_pwd = Label(self, text="Password:", relief='groove', padx=25)
        self.lbl_pwd.grid(row=2, column=1, columnspan=2)
        self.entry_pwd = Entry(self, width=50, show='*')
        ...
        self.btn_Forget = Button(self, text="Password Forgotten ?", command=self.ForgetPwd)
        self.btn_Forget.grid(row=3, column=3, columnspan=2)
    ...
    def initialize(self):
        ...
        #Game2:
        self.buttonGame2 = Button(self,text='Wallpaper', bg='grey', width=15,height=5,command=self.ShowWallpaper)
        self.buttonGame2.grid(row=1, column=3)
  
        #Game3:
        self.buttonGame3 = Button(self,text='Security Camera', bg='grey', width=15,height=5,command=self.SecurityCam)
        self.buttonGame3.grid(row=3, column=1)
        textGame3 = Label(self, text="", state="disabled").grid(row=2, column=2)

        #Game4:
        self.buttonGame4 = Button(self,text="Disengage \nLockdown",bg='red', width=15,height=5,command=self.DisengageLockdown)
    ...
    def SecurityCam(self):
        if self.running_game3 == 0:
            ...
            self.top.btn1 = Button(self.top, text='Ok', command=self.Show_SecurityCam)
            self.top.btn1.grid(row=2, column=1, columnspan=2)

            self.top.btn2 = Button(self.top, text='Quit', command=self.close_SecurityCam)
            self.top.btn2.grid(row=2, column=2, columnspan=2)

            self.top.ent1 = Entry(self.top, width=4, bg='white', textvariable=self.CodeValue)
            self.top.ent1.grid(row=1, column=1)
            
            self.top.ent2 = Entry(self.top, width=4, bg='white', textvariable=self.CodeValue2)
            self.top.ent2.grid(row=1, column=2)
            
            self.top.ent3 = Entry(self.top, width=4, bg='white', textvariable=self.CodeValue3)
            self.top.ent3.grid(row=1, column=3)
            
            self.top.ent4 = Entry(self.top, width=4, bg='white', textvariable=self.CodeValue4)
            self.top.ent4.grid(row=1, column=4)

            self.top.lbl = Label(self.top, text='Are you \'phone connected\' ?', width=30)
            self.top.lbl.grid_remove()
            self.top.bindic = Button(self.top, bitmap='question',command=lambda:self.top.lbl.grid(row=4, column=1, columnspan=3))
    ...
    def Show_SecurityCam(self):
        if (self.top.ent1.get() == str(2)) and (self.top.ent2.get() == str(8)) and (self.top.ent3.get() == str(9)) and (self.top.ent4.get() == str(4)):
            ...
            self.top.lbl_end = Label(self.top, text='video link ?', width=30)
            ...
            self.buttonGame3.config(text="Security Camera", bg='green')
