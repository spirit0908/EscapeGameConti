...
def step1(master_g1,entry1):
    ...
    if user_answer.lower() == s_step1:
        label2 = Label(master_g1, text="Well done !                ")
        label2.grid(row=1, column=0, columnspan=3)
        entry1.config(state="disabled")
        step2(master_g1)
    else:
        label2 = Label(master_g1, text="Wrong answer")
        label2.grid(row=1, column=0, columnspan=3)
    
def step2(master_g1):
    label3 = Label(master_g1, text="What movie do you hate the most ?")
    label3.grid(row=3, column=0)
    entry2 = Entry(master_g1, width=50)
    entry2.grid(row=3, column=1, columnspan=3)
    button2 = Button(master_g1,text="Ok", command=lambda:answer_step2(master_g1,entry2)).grid(row=3,column=4, columnspan=1)
...
def answer_step2(master_g1, entry2):
    ...
    if user_answer.lower() == s_step2.lower():
        label4 = Label(master_g1, text="Well done !                ")
        label4.grid(row=4, column=0, columnspan=3)
        entry2.config(state="disabled")
        step3(master_g1)
    else:
        label4 = Label(master_g1, text="Wrong answer")
        label4.grid(row=4, column=0, columnspan=3)

def step3(master_g1):
    label5 = Label(master_g1, text="Who is the reindeer of the month ?")
    label5.grid(row=5, column=0)
    entry3 = Entry(master_g1, width=50)
    entry3.grid(row=5, column=1, columnspan=3)
    button3 = Button(master_g1,text="Ok", command=lambda:answer_step3(master_g1,entry3)).grid(row=5,column=4, columnspan=1)
...
def answer_step3(master_g1, entry3):
    ...
    if user_answer.lower() == s_step3.lower():
        label6 = Label(master_g1, text="Well done !                ")
        label6.grid(row=6, column=0, columnspan=3)
        entry3.config(state="disabled")
        step4(master_g1)
    else:
        label6 = Label(master_g1, text="Wrong answer")
        label6.grid(row=6, column=0, columnspan=3)

def step4(master_g1):
    label7 = Label(master_g1, text="What is you favorite dish ?")
    label7.grid(row=7, column=0)
    entry4 = Entry(master_g1, width=50)
    entry4.grid(row=7, column=1, columnspan=3)
    button4 = Button(master_g1,text="Ok", command=lambda:answer_step4(master_g1,entry4)).grid(row=7,column=4, columnspan=1)
...
def game1_end(master_g1):
    label9 = Label(master_g1, text="Welcome Santa ! Good to see you again.")
    label9.grid(row=8, column=0, columnspan=3)
    label10 = Label(master_g1, text="Your new password is: SwoftwarePlatform")
    label10.grid(row=9, column=0, columnspan=3)
    button5 = Button(master_g1,text="Finish", command=lambda:close(master_g1)).grid(row=10,column=1, columnspan=1)
...
def main():
    ...
    ## Login ##
    label1 = Label(master_g1, text="What is your favorite chocolate ?")
    label1.grid(row=0, column=0)
    entry1 = Entry(master_g1, width=50)
    entry1.grid(row=0, column=1, columnspan=3)

    button1 = Button(master_g1,text="Ok", command=lambda:step1(master_g1, entry1)).grid(row=0, column=4,columnspan=1)
