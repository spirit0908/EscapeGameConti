...
def step1(master_g2,entry1,entry2):
    ...
    if (user_answer.lower() == s_step1) and (user_answer2.lower() == s_step2):
        label5.config(text='Cameras are now unlocked')
        label5.grid(row=4, column=1, columnspan=3)
        entry1.config(state="disabled")
        entry2.config(state="disabled")
        game2_end(master_g2)
    else:
        label5.config(text="Wrong answer")
        label5.grid(row=4, column=1, columnspan=1)

def game2_end(master_g2):
    #messagebox.showinfo("Info","You can click on \"Finish\"")
    #p = PhotoImage(file="Qrcode.gif")
    #canvas = Canvas(master_g2, width='500', height='400')
    #canvas.grid(row=8, column=1)
    #canvas.create_image(0,0,anchor=NW,image=p)
    label10 = Label(master_g2, text="You can click on \"Finish\"", image=qrcode)
    label10.grid(row=8, column=1, columnspan=1)
...
def main():
    ### Main ###   
    master_g2 = Tk()
    master_g2.title('Unlock Security Cameras')
